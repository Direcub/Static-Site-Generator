...
def extract_title(markdown):
    markdownList = markdown.split("\n")
    if markdownList[0].startswith("# "):
        shortenedHeader = markdownList[0][2:]
        return shortenedHeader
    else:
        raise Exception
...
def generate_page(from_path, template_path, dest_path):
    ...
    converted_html = markdown_to_html_node(file_contents)
    html_string = converted_html.to_html()
...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, root_dir_path_content=None):
    ...
    for path in os.listdir(dir_path_content):
        ...
        if os.path.isfile(full_path) and full_path.endswith(".md"):
            relative_path = os.path.relpath(full_path, root_dir_path_content)
            print(f"Relative Path: {relative_path}")
            output_path = os.path.join(dest_dir_path, relative_path)
            ...
            generate_pages_recursive(full_path, template_path, dest_dir_path, root_dir_path_content)